...
---
title: "CLI Reference"
description: "Command-line options for .ai.soc"
---

## Basic Usage

```bash
soc --target <target> [options]
```
...
<ParamField path="--mount" type="string">
  ...
  .ai.soc copies local `--target` directories into the sandbox one file at a time, which stalls on very large trees. When a local target exceeds the copy limit (see `STRIX_MAX_LOCAL_COPY_MB`, default 1024 MB) .ai.soc exits early and asks you to re-run with `--mount`.
...
soc --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
soc --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
soc -n --target ./ --scan-mode quick

# Force diff-scope against a specific base ref
soc -n --target ./ --scan-mode quick --scope-mode diff --diff-base origin/main

# Multi-target white-box testing
soc -t https://github.com/org/app -t https://staging.example.com
