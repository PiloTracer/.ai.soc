...
description: "Run .ai.soc security scans on every pull request"
---

Integrate .ai.soc into your GitHub workflow to catch vulnerabilities before they reach production.
...
<Note>
For pull_request workflows, .ai.soc automatically uses changed-files diff-scope in CI/headless runs. If diff resolution fails, ensure full history is fetched (`fetch-depth: 0`) or set `--diff-base`.
</Note>
