...
---
title: "Configuration"
description: "Environment variables for .ai.soc"
---

Configure .ai.soc using environment variables or a config file.
...
<ParamField path="SOC_MEMORY_COMPRESSOR_TIMEOUT" default="30" type="integer">
...
<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, .ai.soc keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, .ai.soc still writes complete run telemetry locally to:

```bash
strix_runs/<run_name>/events.jsonl
```

When remote vars are set, .ai.soc dual-writes telemetry to both local JSONL and the remote OTEL endpoint.
...
<ParamField path="STRIX_MAX_LOCAL_COPY_MB" default="1024" type="integer">
  Maximum size (in MB) of a local directory target that .ai.soc will copy into the sandbox file-by-file. Larger targets exit early with a suggestion to use `--mount` instead. Set to `0` to disable the check.
</ParamField>

## Sandbox Configuration

<ParamField path="SOC_SANDBOX_EXECUTION_TIMEOUT" default="120" type="integer">
  Maximum execution time in seconds for sandbox operations.
</ParamField>

<ParamField path="SOC_SANDBOX_CONNECT_TIMEOUT" default="10" type="integer">
  Timeout in seconds for connecting to the sandbox container.
</ParamField>

## Config File

.ai.soc stores configuration in `~/.strix/cli-config.json`. You can also specify a custom config file:

```bash
soc --target ./app --config /path/to/config.json
```
...
export SOC_SANDBOX_EXECUTION_TIMEOUT="300"
